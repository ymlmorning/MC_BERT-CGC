...
def sigmoid_focal_loss(inputs, targets, alpha=0.45, gamma=1, reduction="none"):
...
def poly_loss(inputs, targets, alpha=0.5, reduction="none"):
    #labels_onehot = F.one_hot(targets.to(torch.int64), num_classes=len(targets.to(torch.int64))).to(device=inputs.device,dtype=inputs.dtype)
    pt = torch.sum(targets * F.softmax(inputs, dim=-1), dim=-1)
    ce_loss = F.binary_cross_entropy_with_logits(inputs, targets, reduction="none")
    #p = torch.sigmoid(inputs)
    loss=ce_loss+alpha*(1-pt)
    return loss
...
class CNNNer(nn.Module):
    ...
    def forward(self, input_ids, bpe_len, indexes, matrix):
        ...
        if self.training:
            ...
            #flat_loss=poly_loss(flat_scores, flat_matrix.float())
